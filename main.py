...
async def on_message(message):
    ...
    if message.content.startswith("go"):
        await message.channel.send("go umass!")
...
async def all_words(ctx, lang_name):
    s_db = db.get(query.func == "lang")[0]
    if lang_name in s_db.keys():
        all_terms = s_db[lang_name]
        all_terms_string = "__**" + lang_name.upper() + "**__\n"
        for key, value in all_terms.items():
            all_terms_string += f"`{key}`: {value}\n"
        await ctx.send(all_terms_string)
    else:
        await ctx.send("Language does not exist.")
...
async def import_lang(ctx, *, text):
    try:
        def check_reply(m):
            return m.author == ctx.author and m.channel == ctx.channel
        await ctx.send("Enter a two-character code for this language!")
        msg = await client.wait_for("message", timeout=60.0, check=check_reply)
        name = msg.content.lower()
        while db.search(where(name).exists()):
            await ctx.send("This language already exists, choose a new name or reply `cancel` and use add_term or delete_set.")
            msg = await client.wait_for("message", timeout=60.0, check=check_reply)
            name = msg.content
            if name == "cancel":
                await ctx.send("Set creation cancelled.")
        ...
        db.upsert({name: study_set}, query.func == "lang")
        await ctx.send(f"Language set {name} created!")
    except commands.errors.MissingRequiredArgument:
        await ctx.send("Didn't see any text to import! (format: term1,def1;term2,def2; ...)")
    except asyncio.TimeoutError:
        await ctx.send(content="Language set creation timed out.", reference=ctx.message)

# toggle enable or disable certain language
@client.command()
async def toggle(ctx, lang_code):
    if db.search(where(lang_code).exists()):
        global enabled_languages
        if lang_code in enabled_languages:
            enabled_languages.remove(lang_code)
            db.update({"enabled":enabled_languages}, query.func == "lang")
            await ctx.send("Deactivated prompts for `{lang_code}`.")
        else:
            enabled_languages.append(lang_code)
            db.update({"enabled":enabled_languages}, query.func == "lang")
            await ctx.send(f"Activated prompts for `{lang_code}`.")
    else:
        await ctx.send("Couldn't find that language!")
